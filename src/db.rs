...
impl Transaction {
    ...
    pub fn conn(&self) -> BorsResult<&pg::Connection> {
        if !self.slot.filled() {
            let conn = try!(self.app.database.get().chain_err(|| {
                "failed to get a database connection"
            }));
            self.slot.fill(Box::new(conn)).ok().unwrap();
        }
        Ok(&***self.slot.borrow().unwrap())
    }

    fn tx<'a>(&'a self) -> BorsResult<&'a (GenericConnection + 'a)> {
...
pub trait RequestTransaction {
    ...
    fn db_conn(&self) -> BorsResult<&pg::Connection>;
    ...
    fn tx(&self) -> BorsResult<&GenericConnection>;
